...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(
            "keyword",
            OpenApiTypes.STR,
            description="キーワード検索（タイトル、概要、オーナー名）",
            required=False,
        ),
        OpenApiParameter(
            "category",
            OpenApiTypes.STR,
            description="カテゴリーのslug（カンマ区切り可 例: web,ai）",
            required=False,
        ),
        OpenApiParameter(
            "technology",
            OpenApiTypes.STR,
            description="技術名（カンマ区切り可 例: python,react）",
            required=False,
        ),
        OpenApiParameter(
            "status",
            OpenApiTypes.STR,
            description="進行ステータス（opening, ongoing, completed）",
            required=False,
        ),
    ],
)
